...
class Consensus_Reranking:
    ...
    def load_anno_ref_hypo(self):
        ...
        self.anno_list_ref = np.load(fname_anno_list_ref).tolist()
        # Load hypo annotation list
        fname_hypo_list = self.conf.fname_hypo_list
        self.anno_list_hypo = np.load(fname_hypo_list).tolist()
    ...
    def find_NNimg(self):
        ...
        if os.path.isfile(fname_NNimg_list):
            self.NNimg_list = np.load(fname_NNimg_list).tolist()
        ...
        if not (os.path.isfile(fname_img_feat_tr) and os.path.isfile(fname_img_feat_te) ):
            ...
            dct_feat = np.load(fname_dct_feat).tolist()
            ...
            feat_arr_tr = np.load(fname_img_feat_tr)
            feat_arr_te = np.load(fname_img_feat_te)
    ...
    def consensus_rerank(self, method='cider', flag_eval=True):
        ...
        for (ind_te, anno) in enumerate(anno_list_reranked):
            ...
            anno[key_reranking] = [sentences_gen[x] for x in arg_sim]

            if (ind_te + 1) % self.conf.num_show_finished == 0:
                logger.info('%d image reranking finished' % (ind_te + 1) )
        
        self.anno_list_reranked = anno_list_reranked
        ...
        np.save(fname_anno_list_reranked, anno_list_reranked)
        ...
        if flag_eval:
            ...
            self._anno_genS2coco(anno_list_reranked, key_reranking, 0, fname_coco_json)
    ...
    def _anno_genS2coco(self, anno_list, key, ind_used, fname_COCO_json):
        ...
        for anno in anno_list:
            gen_sentence = anno[key][ind_used]
            anno_coco = dict()
